...
import { getInstitutions, searchInstitutionsByEmail } from "request/accounts";
...
class Institutional extends React.Component {
    ...
    async getAccounts(page) {
        const { success, data } = await getInstitutions(page);
        if (success) {
            const { results } = data;
    ...
    async handleSearch() {
        ...
        if (searchEmail) {
            const { data, success } = await searchInstitutionsByEmail(
    ...
    render() {
        ...
        accounts.map(val => {
            let handleVal = {
                ...val,
                account: (
                    <Link to={`/institutional/edit?userId=${val.id}`}>
