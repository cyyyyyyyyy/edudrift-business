...
export const renderFrom = (list, props) => {
  let arr = [];
  const { t, values, errors, handleChange, handleBlur, setFieldValue } = props;
  ...
  const renderSelect = val => {
    let select = [];
    if (val && val.options) {
      select = val.options.filter(data => data.value === values[`${val.key}`]);
    }
    return (
      <li key={val.key}>
    ...
    return (
      <li key={val.key}>
        ...
        {val.edit ? (
          ...
          <span>{moment(values[`${val.key}`]).format("YYYY/MM/DD")}</span>
        )}
      </li>
    );
  };
