...
import { contact_method, gender, Currency } from "utils/select-options";
import { renderFrom } from "./render-from";
import { getInstitutionsById, updateInstitutions } from "../request/accounts";
...
const formList1 = t => [
    { name: t("Full Name of Institution"), key: "full_name", type: "input" },
    { name: t("Short Name of Institution"), key: "short_name", type: "input" },
...
@withI18n()
class InstitutionalEdit extends React.Component {
    render() {
...
const From = withFormik({
    ...
    handleSubmit: async (values, { setSubmitting, props }) => {
        ...
        const { success } = await updateInstitutions(userId, data);
        if (success) {
...
export default class InstitutionalEditRouter extends React.Component {
    state = {
        data: {}
    };
    ...
    componentWillMount() {
        ...
        const userId = queryString.parse(location.search).userId;
        getInstitutionsById(userId).then(({ data }) => {
            this.setState({
                data: data
            });
        });
